...
if(isset($_REQUEST['sbt-dpt'])) {
  ...
  $insert_department = mysqli_query($conn,"insert into tbl_department set department='$deptname', email='$email', status='$status'");

  if($insert_department > 0) {
    if (!empty($email)) {
      $mail = getMailerInstance();
      $mail->addAddress($email);
      $mail->Subject = 'Department Added';
      $mail->Body = "The department '$deptname' has been successfully added to the system.";
...
<?php include('include/side-bar.php'); ?>
  ...
  <div class="card mb-3">
      ...
      <div class="form-group row">
      <label class="col-lg-4 col-form-label" for="email">Department Email</label>
       <div class="col-lg-6">
      <input type="email" name="email" id="email" class="form-control" placeholder="Enter Department Email">
       </div>
      </div> 
      <div class="form-group row">
